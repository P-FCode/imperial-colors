...
public class TributacaoProdutoMapping : IEntityTypeConfiguration<TributacaoProduto>
{
    public void Configure(EntityTypeBuilder<TributacaoProduto> builder)
    {
        builder.ToTable("tributacao_produtos");

        // Chave primária compartilhada com produtos (relação 1:1 opcional).
        builder.HasKey(t => t.ProdutoId);
        builder.Property(t => t.ProdutoId).HasColumnName("produto_id").ValueGeneratedNever();
        ...
        builder.Property(t => t.ValorIpiFixo).HasColumnName("valor_ipi_fixo").HasPrecision(10, 2);
        builder.Property(t => t.ExTipi).HasColumnName("ex_tipi").HasMaxLength(3);

        builder.Property(t => t.UnidadeTributavel).HasColumnName("unidade_tributavel").HasMaxLength(10);
        builder.Property(t => t.FatorConversao).HasColumnName("fator_conversao").HasPrecision(10, 4);
        builder.Property(t => t.GtinTributavel).HasColumnName("gtin_tributavel").HasMaxLength(14);
        ...
        builder.HasIndex(t => t.Ncm);

        builder.HasOne(t => t.Produto)
            .WithOne(p => p.Tributacao)
            .HasForeignKey<TributacaoProduto>(t => t.ProdutoId)
